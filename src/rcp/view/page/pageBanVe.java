...
public class pageBanVe extends Composite {
	...
	public pageBanVe(Composite parent, int style) {
		...
		gd_dateTime.heightHint = 28;
		gd_dateTime.widthHint = 150;
		...
		GridData gd_btnTim = new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1);
		gd_btnTim.heightHint = 30;
		gd_btnTim.widthHint = 70;
		btnTim.setLayoutData(gd_btnTim);
		btnTim.setImage(SWTResourceManager.getImage(pageBanVe.class, "/rcp/view/page/zoom_16x16.png"));
		...
		GridData gd_tablePhim = new GridData(SWT.FILL, SWT.FILL, true, true, 2, 1);
		...
		btnChon.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					if (!tableSuatChieu.getSelection()[0].getText(7).equals("Đã chiếu"))
						Window.open(new frmChonGhe(getDisplay(), tableSuatChieu.getSelection()[0].getText(1)));
					else
						Message.show("Suất chiếu này đã chiếu", "Lỗi", SWT.OK | SWT.ICON_ERROR, getShell());
				} catch (Exception ex) {
		...
		btnChon.setImage(SWTResourceManager.getImage(pageBanVe.class, "/rcp/view/popup/yes_16x16.png"));
	...
	public void traCuu() {
		try {
			ArrayList<Phim> aPhim = SuatChieuController
					.traCuuPhim_theoNgay(DateF.toDate(dateTime.getYear(), dateTime.getMonth(), dateTime.getDay()));
			tablePhim.removeAll();
			for (Phim p : aPhim) {
	...
	public void traCuuSuatChieu() {
		...
		try {
			...
			for (SuatChieu sc : a) {
				...
				if (sc.getTrangThai() == 1)
					trangThai = "Đang chiếu";
				if (sc.getTrangThai() == 2)
					trangThai = "Sắp chiếu";
				...
				item.setText(new String[] { String.valueOf(tableSuatChieu.getItemCount()), sc.getMaSuatChieu(),
						localDateFormat.format(sc.getThoiGianChieu()), sc.getDinhDang(), sc.getHinhThuc(),
						sc.getNgonNgu(), sc.getPhong(), trangThai });
			}
