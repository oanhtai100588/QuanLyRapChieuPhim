...
public class frmBanVe extends Shell {
	...
	public frmBanVe(Display display, String maSuatChieu, ArrayList<String> dsGheDaChon) {
		...
		label_1.setImage(SWTResourceManager.getImage(frmBanVe.class, "/rcp/view/popup/imgVe_64p.png"));
		...
		txtNhanVien.setText(Settings.currentEmpId);
		...
		lblTnThcn.setBounds(277, 102, 82, 15);

		txtMaKhachHang = new Text(this, SWT.BORDER);
		txtMaKhachHang.setBounds(375, 99, 141, 24);
		...
		composite.setBounds(0, 515, 541, 56);

		btnLuu = new Button(composite, SWT.NONE);
		btnLuu.setImage(SWTResourceManager.getImage(frmBanVe.class, "/rcp/view/page/save_16x16.png"));
		...
		btnLuu.setText("Lưu");
		btnLuu.setBounds(337, 10, 94, 33);
		btnLuu.setEnabled(false);

		Button btnHuy = new Button(composite, SWT.NONE);
		btnHuy.setImage(SWTResourceManager.getImage(frmBanVe.class, "/rcp/view/page/cancel_16x16.png"));
		...
		btnHuy.setBounds(437, 10, 94, 33);

		txtMaSuatChieu = new Text(this, SWT.BORDER | SWT.READ_ONLY);
		txtMaSuatChieu.setBounds(375, 144, 141, 24);
		...
		txtSuatPhim.setBounds(375, 192, 141, 24);
		...
		txtPhong.setBounds(375, 242, 141, 24);

		dateTime = new DateTime(this, SWT.BORDER | SWT.READ_ONLY | SWT.DROP_DOWN);
		dateTime.setBounds(109, 144, 141, 24);
		dateTime.setEnabled(false);

		tableVe = new Table(this, SWT.BORDER | SWT.FULL_SELECTION);
		tableVe.setBounds(23, 372, 493, 129);
		...
		txtTongTien.setBounds(375, 292, 141, 24);
		...
		btnKiemTra.setImage(SWTResourceManager.getImage(frmBanVe.class, "/rcp/view/popup/yes_16x16.png"));
		...
		btnKiemTra.setBounds(408, 330, 108, 30);
		...
		if (!kiemTraTonTai(txtMaKhachHang.getText())) {
			...
			try {
				if (VeController.layMaKhuyenMai(txtMaKhachHang.getText()) == null)
					...
					txtKhuyenMai.setText(VeController.layMaKhuyenMai(txtMaKhachHang.getText()));
				hienVe();
	...
	public void hienVe() {
		...
		for (Ve v : arrVe) {
			...
			item.setText(new String[] { v.getMaGhe().substring(3, 6), String.valueOf(v.getGiaVe()),
					String.valueOf(v.getDiemTichLuy()) });
			tongTien += v.getGiaVe();
		}
		tableVe.select(0);
	...
	public void them() {
		try {
			if (VeController.them(arrVe)) {
				...
				Message.show("Thêm vé không thành công", "Lỗi", SWT.OK | SWT.ICON_ERROR, getShell());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
